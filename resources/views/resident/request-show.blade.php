...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <a href="{{ route('resident.requests') }}" class="btn-ghost mb-4">&larr; Back to Requests</a>
    ...
    <div class="card">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-6 sm:gap-4 text-sm">
            ...
            <div>
                <span class="text-gray-600 dark:text-gray-400">Queue Position</span>
                <p class="font-medium">{{ $request->queue_position ?? 'Waiting' }}</p>
            </div>
            <div>
                <span class="text-gray-600 dark:text-gray-400">Total Weight</span>
                <p class="font-medium">{{ number_format((float) $request->total_weight, 2) }}</p>
            </div>
            <div>
