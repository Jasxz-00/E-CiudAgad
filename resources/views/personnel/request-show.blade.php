                ...
                <div class="space-y-4">
                    @if($request->resident?->idVerifications->isNotEmpty())
                        @foreach($request->resident->idVerifications as $idv)
                            @php
                                $idFileUrl = $idv->file_path ? Storage::disk('private')->url($idv->file_path) : null;
                                $isImage = $idv->file_type && str_starts_with($idv->file_type, 'image/');
                            @endphp
                            <div class="p-3 bg-gray-50 dark:bg-gray-950 rounded-xl">
                                ...
                                <p class="text-xs text-gray-600 dark:text-gray-400 mb-2">{{ __('common.id_number') }}: {{ $idv->id_number }}</p>
                                @if($idFileUrl)
                                    @if($isImage)
                                        <button type="button" @@click="openLightbox = '{{ $idFileUrl }}'" class="inline-flex items-center gap-1 text-xs text-primary-700 dark:text-primary-400 hover:underline">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                                            {{ __('common.view_photo') }}
                                        </button>
                                    @else
                                        <a href="{{ $idFileUrl }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1 text-xs text-primary-700 dark:text-primary-400 hover:underline">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                                            {{ __('common.view_file') }}
                                        </a>
                                    @endif
                                    <span class="text-xs text-gray-600 dark:text-gray-400 ml-2">({{ number_format($idv->file_size / 1024, 1) }} KB)</span>
                                @else
            ...
            <div class="card">
                ...
                @if($request->resident?->idVerifications->isNotEmpty())
                    ...
                    <div class="text-sm space-y-1">
                        ...
                        <p><span class="text-gray-600 dark:text-gray-400">{{ __('common.id_number') }}:</span> {{ $idv->id_number }}</p>
