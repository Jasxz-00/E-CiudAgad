...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <x-section-header title="Admin Accounts" description="Manage administrator accounts"
        actionUrl="{{ route('admin.users.create') }}?return=accounts" actionLabel="Add Admin"
    ...
    <form method="GET" action="{{ route('admin.accounts.index') }}" class="mb-6">
        <x-card>
            <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('admin.accounts.index') }}" class="btn-ghost flex-1 text-center">Clear</a>
                </div>
            </div>
        </x-card>
    </form>
            ...
            <tr>
                ...
                <td>
                    <div class="flex gap-3 items-center">
                        ...
                        <a href="{{ route('admin.users.edit', $user) }}?return=accounts" class="text-primary-700 dark:text-primary-400 hover:underline text-sm inline-flex items-center min-h-[36px]">Edit</a>
                    </div>
                </td>
            </tr>
            @empty
            <tr>
                <td colspan="5" class="text-center py-8 text-gray-600 dark:text-gray-400">No admin accounts found.</td>
            </tr>
            @endforelse
        </x-table>
        @if($users->hasPages())
